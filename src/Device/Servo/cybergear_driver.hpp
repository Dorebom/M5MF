...
public:
    ...
    bool set_serial_id(twai_message_t &msg, XiaomiCyberGearStatus &status) {
        if (!status.is_connected) {
            if ((msg.identifier & 0x000000FF) == 0xFE) {
    ...
    void update_status(twai_message_t &rx_msg, XiaomiCyberGearStatus &status) {
        ...
        if (can_id != status.can_id) {
            return;
        }

        switch (msg_cmd) {
            case CMD_GET_ID:
                set_serial_id(rx_msg, status);
                break;
            case CMD_RESPONSE_MOTION_STATE:
                update_ctrl_state(rx_msg, status);
                break;
            case CMD_RAM_READ:
                update_ram_data(rx_msg, status);
                break;
